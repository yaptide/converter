...
class ScoringFilter:
    ...
    def __str__(self) -> str:
        return self.template.format(
            name=self.name, rules="".join([self.rule_template.format(*rule) for rule in self.rules])
        )
...
class QuantitySettings:
    ...
    def __str__(self) -> str:
        return "Settings" + "".join(
            [
                self.settings_dict[key].format(**{key: value})
...
class DetectConfig:
    ...
    detectors: list[ScoringDetector] = field(
    ...
    outputs: list[ScoringOutput] = field(
        default_factory=lambda: [
    ...
    def __str__(self):
        return "\n".join(
            [
                ...
                "\n".join(
                    [
                        str(quantity.settings)
                        for output in self.outputs
                        for quantity in output.quantities
                        if quantity.settings
                    ]
                ),
