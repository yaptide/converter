...
from converter.fluka.helper_parsers.material_parser import parse_materials, assign_materials_to_regions, set_custom_ionisation_potential
...
class FlukaParser(Parser):
    ...
    def parse_configs(self, json: dict) -> None:
        ...
        materials, compounds = parse_materials(json["materialManager"]["materials"], json["zoneManager"])
        self.input.materials = list(material for material in materials.values() if material.fluka_name.startswith(("MAT", "COM")))
        self.input.compounds = list(compound for compound in compounds.values() if compound.fluka_name.startswith("COM"))
