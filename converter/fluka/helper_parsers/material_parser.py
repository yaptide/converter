...
from converter.fluka.predefined_materials import PREDEFINED_MATERIALS, PREDEFINED_COMPOUNDS
...
def parse_materials(materials_json, zones_json: dict) -> (dict[str, FlukaMaterial], dict[str, FlukaCompound]):
    ...
    for zone in zones:
        if "customMaterial" in zone and "density" in zone["materialPropertiesOverrides"]:
            materials_list.append((zone["customMaterial"]["uuid"], zone["customMaterial"]["icru"], zone["materialPropertiesOverrides"]["density"]))
...
def assign_materials_to_regions(materials: dict[str, FlukaMaterial], regions: dict[str, FlukaRegion], zones_json: dict) -> list[MaterialAssignment]:
...
def set_custom_ionisation_potential(materials: dict[str, FlukaMaterial], custom_materials: list) -> list[IonisationPotential]:
    ...
    for material in custom_materials:
        if "ionisationPotential" in material:
            ionisation_potentials.append(IonisationPotential(
